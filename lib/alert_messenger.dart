...
class AlertMessengerState extends State<AlertMessenger>
  ...
  final alertWidget = ValueNotifier<Alert?>(null);
  ...
  Future<void> showAlert({required Alert alert}) async {
    if (alertWidget.value != null &&
        alert.priority.value > alertWidget.value!.priority.value) {
      alerts.removeAt(0);
      await controller.reverse();
    }
    alerts.add(alert);
    alerts.sort((a, b) => b.priority.value.compareTo(a.priority.value));
    alertWidget.value = alerts.isNotEmpty ? alerts.first : null;
    controller.forward();
  }
  ...
  Future<void> hideAlert() async {
    ...
    if (alerts.isNotEmpty) {
      alerts.removeAt(0);
      alertWidget.value = alerts.isNotEmpty ? alerts[0] : null;
      if (alertWidget.value != null) controller.forward();
    }
  }
